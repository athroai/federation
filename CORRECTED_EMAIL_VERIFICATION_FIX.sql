-- 🚀 CORRECTED EMAIL VERIFICATION FIX
-- This addresses the syntax error and fixes email verification

-- 1. Add missing INSERT policy (with proper syntax)
DO $$
BEGIN
  IF NOT EXISTS (
...
-- 10. Show all current policies for verification
SELECT 
  'POLICIES_CHECK' as status,
  policyname,
  cmd,
  with_check
FROM pg_policies 
