    ...
    try {
      ...
      if (!isImage) {
        try {
          ...
          // Document processing result logged internally
...
| Learn about culture and customs | ${options['learnCulture']} |`;
          ...
          return;
        default:
          // Handle any other bracketed option as a generic request
          userMessage = `I want help with: ${option}`;
      }
  ...
  const extractClickableOptions = (content: string): string[] => {
    ...
    // Match patterns like [Option Name] - extract ALL bracketed text
    const optionRegex = /\[([^\]]+)\]/g;
    let match;
                ...
                >
                  ...
                  {isWelcomeMessage ? (
                    <div style={{ lineHeight: '1.6' }}>
                      ...
                      {(() => {
                        // Debug logging removed to prevent excessive console spam
                                                  ...
                                                  // Welsh debug logging removed to prevent console spam
                    ...
                    <div
                      dangerouslySetInnerHTML={{ 
                        __html: (() => {
                          // Process brackets BEFORE markdown rendering to avoid interference
                          const preprocessedContent = displayContent.replace(
                            /\[([^\]]+)\]/g, 
                            '<BRACKET_PLACEHOLDER_START>$1<BRACKET_PLACEHOLDER_END>'
                          );
                          
                          // Render markdown
                          const renderedHtml = renderMessage(preprocessedContent);
                          
                          // Convert placeholders to clickable spans
                          return renderedHtml.replace(
                            /<BRACKET_PLACEHOLDER_START>([^<]+)<BRACKET_PLACEHOLDER_END>/g,
                  ...
                  <div
                    dangerouslySetInnerHTML={{ 
                      __html: (() => {
                        // For non-bilingual messages, also process brackets to make them clickable
                        // if they exist outside of welcome/response contexts
                        const hasBrackets = /\[([^\]]+)\]/.test(displayContent);
                        
                        if (hasBrackets && !hasClickableOptions && !isWelcomeMessage) {
                          // Process brackets BEFORE markdown rendering for consistency
                          const preprocessedContent = displayContent.replace(
                            /\[([^\]]+)\]/g, 
                            '<BRACKET_PLACEHOLDER_START>$1<BRACKET_PLACEHOLDER_END>'
                          );
                          ...
                          return renderedHtml.replace(
                            /<BRACKET_PLACEHOLDER_START>([^<]+)<BRACKET_PLACEHOLDER_END>/g,
                            '<span class="inline-bracket-option" data-option="$1" data-index="' + index + '" style="background: rgba(79, 195, 138, 0.2); color: #4fc38a; padding: 2px 6px; border-radius: 4px; cursor: pointer; border: 1px solid #4fc38a; display: inline; margin: 0 2px; transition: all 0.2s ease; font-weight: 500;">$1</span>'
                          );
                        } else {
                          return renderMessage(displayContent);
                        }
                      })()
                    }}
                    style={{ 
                    ...
                    onClick={(e) => {
                      const target = e.target as HTMLElement;
                      if (target.classList.contains('inline-bracket-option')) {
                        const option = target.getAttribute('data-option');
                        const msgIndex = target.getAttribute('data-index');
                        if (option && msgIndex) {
                          handleResponseOptionClick(option, parseInt(msgIndex));
