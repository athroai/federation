...
const ChatInterface: React.FC<ChatInterfaceProps> = ({ athro, onSaveSession, onDeleteSession, onViewHistory, canSaveSession, canDeleteSession, sessionAction, onSessionActionComplete, currentSessionId, onLoadSession, isCreatingSession }) => {
  ...
  // Track Side by Side mode for persistent dual-column responses
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 🔄 SIDE BY SIDE MODE: Create explicit dual-column context
      if (isSideBySideMode && selectedLanguage) {
        console.log('🔄 APPLYING SIDE BY SIDE MODE - Language:', selectedLanguage, 'Mode:', isSideBySideMode);
        languageContext = `CRITICAL DUAL-COLUMN INSTRUCTION: The user has selected "Side by Side" mode.

YOU MUST RESPOND IN DUAL-COLUMN FORMAT FOR ALL SUBSEQUENT MESSAGES.

MANDATORY FORMAT:
| **English** | **${selectedLanguage}** |
|-------------|------------|
| [English content] | [${selectedLanguage} content] |
| [Next English line] | [Next ${selectedLanguage} line] |

STRICT REQUIREMENTS:
1. EVERY response must be in this dual-column table format
2. NO English text outside the table columns
3. NO explanations before or after the table
4. EVERY line of content must appear in BOTH languages
5. Match the table structure exactly with the separator line
6. Include clickable options in BOTH columns: **[English Option]** | **[${selectedLanguage} Option]**
7. Never break from this format - maintain dual columns always

Respond now in DUAL-COLUMN FORMAT ONLY.`;
...
Responda agora APENAS EM PORTUGUÊS.`;
      ...
      const userMessage: ChatMessage = {
        ...
        content: `Please review this ${isPdf ? 'PDF' : 'document'}: ${fileName}`
          ...
          chatService.current.clearCache();
                    ...
                    // Comprehensive language detection to prevent crossover between similar languages
          let context = '';
          
          // Determine current language from selectedLanguage state or athro context
          const currentLanguage = selectedLanguage || (athro.id === 'athro-cymraeg' ? 'Welsh' : 'English');
          
          // Process based on current language (shortened but comprehensive)
          if (hasActualContent) {
            if (currentLanguage === 'Welsh') {
              context = `CAIS DADANSODDI DOGFEN: Mae doiciméad "${fileName}" wedi'i uwchlwytho gan y defnyddiwr ar gyfer adolygiad. Mae gennych fynediad llawn i ddarllen a dadansoddi'r ddogfen hon.`;
            } else if (currentLanguage === 'Irish') {
              context = `IARRATAS ANAILÍSE DOICIMÉID: Tá doiciméad "${fileName}" uaslódáilte ag an úsáideoir le haghaidh athbhreithnithe. Tá rochtain iomlán agat an doiciméad seo a léamh agus a anailísiú.`;
            } else if (currentLanguage === 'Spanish') {
              context = `SOLICITUD DE ANÁLISIS DE DOCUMENTO: El usuario ha subido un documento "${fileName}" para revisión. Tienes acceso completo para leer y analizar este documento.`;
            } else if (currentLanguage === 'Portuguese') {
              context = `SOLICITAÇÃO DE ANÁLISE DE DOCUMENTO: O usuário enviou um documento "${fileName}" para revisão. Você tem acesso completo para ler e analisar este documento.`;
            } else if (currentLanguage === 'Italian') {
              context = `RICHIESTA DI ANALISI DOCUMENTO: L'utente ha caricato un documento "${fileName}" per la revisione. Hai pieno accesso per leggere e analizzare questo documento.`;
            } else if (currentLanguage === 'French') {
              context = `DEMANDE D'ANALYSE DE DOCUMENT: L'utilisateur a téléchargé un document "${fileName}" pour révision. Vous avez un accès complet pour lire et analyser ce document.`;
            } else if (currentLanguage === 'German') {
              context = `DOKUMENTENANALYSE-ANFRAGE: Der Benutzer hat ein Dokument "${fileName}" zur Überprüfung hochgeladen. Sie haben vollen Zugang, um dieses Dokument zu lesen und zu analysieren.`;
            } else {
            ...
            // Error context for different languages  
            if (currentLanguage === 'Welsh') {
              context = `HYSBYSIAD PROSESU DOGFEN: Mae'r defnyddiwr wedi rhannu dogfen o'r enw "${fileName}". Statws prosesu: ${processedContent}`;
            } else if (currentLanguage === 'Irish') {
              context = `FÓGRA PRÓISEÁLA DOICIMÉID: Tá doiciméad "${fileName}" roinnte ag an úsáideoir. Stádas próiseála: ${processedContent}`;
            } else if (currentLanguage === 'Spanish') {
              context = `AVISO DE PROCESAMIENTO: El usuario compartió un documento "${fileName}". Estado: ${processedContent}`;
            } else if (currentLanguage === 'Portuguese') {
              context = `AVISO DE PROCESSAMENTO: O usuário compartilhou um documento "${fileName}". Estado: ${processedContent}`;
            } else {
              context = `DOCUMENT PROCESSING NOTICE: The user shared a document "${fileName}". Status: ${processedContent}`;
            }
          }
  ...
  const handleWelcomeOptionClick = async (option: string, messageIndex: number) => {
    ...
    try {
      ...
      if (['French', 'German', 'Spanish', 'Italian', 'Irish', 'Polish', 'Welsh', 'Greek', 'Turkish', 'Ukrainian', 'Mandarin Chinese', 'Arabic', 'Dutch', 'Hindi', 'Japanese', 'Korean', 'Portuguese', 'Russian', 'Tagalog'].includes(option)) {
        ...
        if (option === 'English Only' || option === 'Saesneg yn unig - English only') {
          ...
          // Activate Side by Side mode for ALL subsequent responses
          console.log('🔄 ACTIVATING SIDE BY SIDE MODE (Welsh)');
          setIsSideBySideMode(true);
          setSelectedLanguage('Welsh'); // Set Welsh as the target language for AthroCymraeg
          
          // Forward to the complete implementation in handleResponseOptionClick
          return handleResponseOptionClick(option, messageIndex);
        } else {
  ...
  const handleResponseOptionClick = async (option: string, messageIndex: number) => {
    ...
    try {
      ...
      // 🔄 SIDE BY SIDE MODE: If in dual-column mode, send ALL requests to AI
      if (isSideBySideMode && selectedLanguage) {
        console.log('🔄 SIDE BY SIDE: Sending option to AI:', option);
        userMessage = `Please help me with "${option}". Respond in dual-column format with English on the left and ${selectedLanguage} on the right.`;
        
        // Add user message and trigger chat manually
        const userMsg: ChatMessage = { role: 'user', content: userMessage };
        const updatedMessages = [...messages, userMsg];
        setMessages(updatedMessages);
        
        // Trigger the chat logic manually
        ...
        // Send to AI with dual-column context
        const dualColumnContext = `CRITICAL DUAL-COLUMN INSTRUCTION: The user has selected "Side by Side" mode.
...
Respond now in DUAL-COLUMN FORMAT ONLY.`;
        ...
        try {
          ...
          console.error('Error in dual-column chat:', error);
        } finally {
          // Clear indicators
          setIsTyping(prev => ({
            ...prev,
...
**What's going on with your homework?** Feel free to share what subject you're working on, what's challenging you, or even just how you're feeling about it. I'm here to listen and help however I can! 🤗`
        ...
        case '[Side by Side]':
          // Activate Side by Side mode for ALL subsequent responses
          console.log('🔄 ACTIVATING SIDE BY SIDE MODE (Languages)');
          setIsSideBySideMode(true);
          
          // No user message - just provide the dual-column table directly
          ...
          const languageOptions: Record<string, Record<string, string>> = {
            ...
            'Dutch': {
              'Vocabulary Practice': 'Woordenschat Oefenen',
              'Grammar Help': 'Grammatica Hulp',
              'Conversation Practice': 'Conversatie Oefenen',
              'Reading Comprehension': 'Begrijpend Lezen',
              'Writing Skills': 'Schrijfvaardigheden',
              'Cultural Insights': 'Culturele Inzichten',
              'startLearning': 'Laten we nu beginnen met leren! Hier zijn enkele manieren waarop ik kan helpen:',
              'whatWork': 'Waar zou je vandaag aan willen werken?',
              'learnWords': 'Leer nieuwe woorden en zinnen',
              'masterGrammar': 'Beheers grammaticaregels en structuren',
              'practiceSpeak': 'Oefen spreken en dialoog',
              'understandTexts': 'Begrijp teksten en literatuur',
              'improveWriting': 'Verbeter je schrijfvaardigheden',
              'learnCulture': 'Leer over cultuur en gebruiken'
            },
            'Hindi': {
              'Vocabulary Practice': 'शब्दावली अभ्यास',
              'Grammar Help': 'व्याकरण सहायता',
              'Conversation Practice': 'बातचीत अभ्यास',
              'Reading Comprehension': 'पठन बोध',
              'Writing Skills': 'लेखन कौशल',
              'Cultural Insights': 'सांस्कृतिक अंतर्दृष्टि',
              'startLearning': 'अब आइए सीखना शुरू करते हैं! यहाँ कुछ तरीके हैं जिनसे मैं आपकी मदद कर सकता हूँ:',
              'whatWork': 'आज आप किस पर काम करना चाहेंगे?',
              'learnWords': 'नए शब्द और वाक्य सीखें',
              'masterGrammar': 'व्याकरण के नियम और संरचनाओं में महारत हासिल करें',
              'practiceSpeak': 'बोलने और संवाद का अभ्यास करें',
              'understandTexts': 'पाठ और साहित्य को समझें',
              'improveWriting': 'अपने लेखन कौशल में सुधार करें',
              'learnCulture': 'संस्कृति और रीति-रिवाजों के बारे में जानें'
            },
            'Japanese': {
              ...
              'Cultural Insights': '文化的洞察',
              'startLearning': 'さあ、学習を始めましょう！お手伝いできる方法をいくつかご紹介します：',
              'whatWork': '今日は何に取り組みたいですか？',
              'learnWords': '新しい単語や表現を学ぶ',
              'masterGrammar': '文法規則と構造をマスターする',
              'practiceSpeak': '話すことと対話を練習する',
              'understandTexts': 'テキストと文学を理解する',
              'improveWriting': 'ライティング能力を向上させる',
              'learnCulture': '文化と習慣について学ぶ'
            },
            'Korean': {
              'Vocabulary Practice': '어휘 연습',
              'Grammar Help': '문법 도움',
              'Conversation Practice': '회화 연습',
              'Reading Comprehension': '독해',
              'Writing Skills': '작문 실력',
              'Cultural Insights': '문화적 통찰',
              'startLearning': '이제 학습을 시작해봅시다! 제가 도울 수 있는 몇 가지 방법들입니다:',
              'whatWork': '오늘은 무엇을 공부하고 싶으신가요?',
              'learnWords': '새로운 단어와 표현을 배우기',
              'masterGrammar': '문법 규칙과 구조 마스터하기',
              'practiceSpeak': '말하기와 대화 연습하기',
              'understandTexts': '텍스트와 문학 이해하기',
              'improveWriting': '작문 능력 향상시키기',
              'learnCulture': '문화와 관습에 대해 배우기'
            },
            'Portuguese': {
              'Vocabulary Practice': 'Prática de Vocabulário',
              'Grammar Help': 'Ajuda Gramatical',
              'Conversation Practice': 'Prática de Conversação',
              'Reading Comprehension': 'Compreensão de Leitura',
              'Writing Skills': 'Habilidades de Escrita',
              'Cultural Insights': 'Insights Culturais',
              'startLearning': 'Agora vamos começar a aprender! Aqui estão algumas maneiras pelas quais posso ajudar:',
              'whatWork': 'No que você gostaria de trabalhar hoje?',
              'learnWords': 'Aprenda novas palavras e frases',
              'masterGrammar': 'Domine regras e estruturas gramaticais',
              'practiceSpeak': 'Pratique fala e diálogo',
              'understandTexts': 'Compreenda textos e literatura',
              'improveWriting': 'Melhore suas habilidades de escrita',
              'learnCulture': 'Aprenda sobre cultura e costumes'
            },
            'Russian': {
              'Vocabulary Practice': 'Практика Словарного Запаса',
              'Grammar Help': 'Помощь с Грамматикой',
              'Conversation Practice': 'Практика Разговора',
              'Reading Comprehension': 'Понимание Прочитанного',
              'Writing Skills': 'Навыки Письма',
              'Cultural Insights': 'Культурные Представления',
              'startLearning': 'Теперь давайте начнем учиться! Вот несколько способов, которыми я могу помочь:',
              'whatWork': 'Над чем вы хотели бы поработать сегодня?',
              'learnWords': 'Изучайте новые слова и фразы',
              'masterGrammar': 'Овладевайте грамматическими правилами и структурами',
              'practiceSpeak': 'Практикуйте речь и диалог',
              'understandTexts': 'Понимайте тексты и литературу',
              'improveWriting': 'Улучшайте навыки письма',
              'learnCulture': 'Изучайте культуру и обычаи'
            },
            'Tagalog': {
              'Vocabulary Practice': 'Pagsasanay sa Bokabularyo',
              'Grammar Help': 'Tulong sa Gramatika',
              'Conversation Practice': 'Pagsasanay sa Pag-uusap',
              'Reading Comprehension': 'Pag-unawa sa Pagbabasa',
              'Writing Skills': 'Kasanayan sa Pagsulat',
              'Cultural Insights': 'Mga Pananaw sa Kultura',
              'startLearning': 'Ngayon ay magsimula na tayo sa pag-aaral! Narito ang ilang paraan kung paano ako makakatulong:',
              'whatWork': 'Ano ang gusto mong pagaralan ngayong araw?',
              'learnWords': 'Matuto ng mga bagong salita at parirala',
              'masterGrammar': 'Makuha ang mga panuntunan at istraktura ng gramatika',
              'practiceSpeak': 'Mag-ensayo ng pagsasalita at pakikipag-usap',
              'understandTexts': 'Unawain ang mga teksto at panitikan',
              'improveWriting': 'Pagbutihin ang inyong kasanayan sa pagsulat',
              'learnCulture': 'Matuto tungkol sa kultura at mga kaugalian'
            }
          };
...
| Learn about culture and customs | ${options['learnCulture']} |`;
          ...
          // Add only the assistant message (dual-column table) directly and return early
          const assistantMsg: ChatMessage = { role: 'assistant', content: responseContent };
  ...
  const extractClickableOptions = (content: string): Array<{option: string, description: string}> => {
    ...
    // PRIORITY 2: Bullet point format with brackets - • [Option] - Description
    if (!foundPattern) {
      const bulletOptionRegex = /•\s*\[([^\]]+)\]\s*-\s*([^\n\r]+)/g;
      const bulletMatches = [...content.matchAll(bulletOptionRegex)];
      
      if (bulletMatches.length >= 2) {
        foundPattern = true;
        bulletMatches.slice(0, 4).forEach(([, optionText, description]) => {
          const cleanOption = optionText.trim();
          const cleanDesc = description.trim();
          
          if (cleanOption.length > 3 && cleanOption.length < 50) {
            options.push({
              option: cleanOption,
    ...
    // PRIORITY 3: Clean ** Text ** - Description format (only if no bracketed options found)
    if (!foundPattern) {
    ...
    // PRIORITY 4: Plain Text - Description format (common pattern)
    if (!foundPattern) {
    ...
    // PRIORITY 5: Language selection options (only if no other patterns found)
    if (!foundPattern) {
    ...
    // PRIORITY 6: Specific language names for selection
    ...
    // PRIORITY 7: Welcome action buttons (only for initial session setup)
          ...
          >
            {messages.map((message, index) => {
              ...
              const isBilingualMessage = message.role === 'assistant' && 
                ((displayContent.includes('| **English** |') && (displayContent.includes('|---|---|') || displayContent.includes('|-------------|------------|'))) ||
                 (displayContent.includes('| **Welsh** |') && (displayContent.includes('|---|---|') || displayContent.includes('|-------------|------------|'))) ||
                 (displayContent.includes('| **French** |') && (displayContent.includes('|---|---|') || displayContent.includes('|-------------|------------|'))) ||
                 (displayContent.includes('| **German** |') && (displayContent.includes('|---|---|') || displayContent.includes('|-------------|------------|'))) ||
                 (displayContent.includes('| **Spanish** |') && (displayContent.includes('|---|---|') || displayContent.includes('|-------------|------------|'))) ||
                 (displayContent.includes('| **Italian** |') && (displayContent.includes('|---|---|') || displayContent.includes('|-------------|------------|'))) ||
                 (displayContent.includes('| **Irish** |') && (displayContent.includes('|---|---|') || displayContent.includes('|-------------|------------|'))) ||
                 (displayContent.includes('| **Polish** |') && (displayContent.includes('|---|---|') || displayContent.includes('|-------------|------------|'))) ||
                 (displayContent.includes('| **Greek** |') && (displayContent.includes('|---|---|') || displayContent.includes('|-------------|------------|'))) ||
                 (displayContent.includes('| **Turkish** |') && (displayContent.includes('|---|---|') || displayContent.includes('|-------------|------------|'))) ||
                 (displayContent.includes('| **Ukrainian** |') && (displayContent.includes('|---|---|') || displayContent.includes('|-------------|------------|'))) ||
                 (displayContent.includes('| **Mandarin Chinese** |') && (displayContent.includes('|---|---|') || displayContent.includes('|-------------|------------|'))) ||
                 (displayContent.includes('| **Arabic** |') && (displayContent.includes('|---|---|') || displayContent.includes('|-------------|------------|'))) ||
                 (displayContent.includes('| **Dutch** |') && (displayContent.includes('|---|---|') || displayContent.includes('|-------------|------------|'))) ||
                 (displayContent.includes('| **Hindi** |') && (displayContent.includes('|---|---|') || displayContent.includes('|-------------|------------|'))) ||
                 (displayContent.includes('| **Japanese** |') && (displayContent.includes('|---|---|') || displayContent.includes('|-------------|------------|'))) ||
                 (displayContent.includes('| **Korean** |') && (displayContent.includes('|---|---|') || displayContent.includes('|-------------|------------|'))) ||
                 (displayContent.includes('| **Portuguese** |') && (displayContent.includes('|---|---|') || displayContent.includes('|-------------|------------|'))) ||
                 (displayContent.includes('| **Russian** |') && (displayContent.includes('|---|---|') || displayContent.includes('|-------------|------------|'))) ||
                 (displayContent.includes('| **Tagalog** |') && (displayContent.includes('|---|---|') || displayContent.includes('|-------------|------------|'))) ||
                          ...
                          <div style={{ marginTop: '1rem' }}>
                            <div className="welcome-options-grid">
                              {[
                                ...
                                { key: 'Dutch', text: 'Dutch', native: 'Nederlands' },
                                { key: 'Hindi', text: 'Hindi', native: 'हिन्दी' },
                                { key: 'Japanese', text: 'Japanese', native: '日本語' },
                                { key: 'Korean', text: 'Korean', native: '한국어' },
                                { key: 'Portuguese', text: 'Portuguese', native: 'Português' },
                                { key: 'Russian', text: 'Russian', native: 'Русский' },
                                { key: 'Tagalog', text: 'Tagalog', native: 'Tagalog' }
                              ].map((language) => (
                                <button
                                  key={language.key}
                                  onClick={(e) => {
                        ...
                        dangerouslySetInnerHTML={{ 
                          __html: renderMessage(
                            displayContent
                              ...
                              // Remove Quick Discussion Starters sections
                              .replace(/\*\*Quick Discussion Starters:\*\*[\s\S]*?• \[.*?\][\s\S]*?• \[.*?\][\s\S]*?• \[.*?\][\s\S]*?• \[.*?\][\s\S]*?/g, '')
                              // Remove Testing Options sections
                              .replace(/\*\*Testing Options:\*\*[\s\S]*?• \[.*?\][\s\S]*?• \[.*?\][\s\S]*?• \[.*?\][\s\S]*?• \[.*?\][\s\S]*?/g, '')
                              // Remove Additional Help Options sections
                              .replace(/\*\*Additional Help Options:\*\*[\s\S]*?• \[.*?\][\s\S]*?• \[.*?\][\s\S]*?• \[.*?\][\s\S]*?• \[.*?\][\s\S]*?/g, '')
                              // Remove bullet point options that are rendered as buttons
                              .replace(/• \[([^\]]+)\][^\n\r]*/g, '')  // Remove • [Option] - description
