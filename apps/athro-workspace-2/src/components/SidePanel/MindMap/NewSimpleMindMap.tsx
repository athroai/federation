...
const NewSimpleMindMap: React.FC<SimpleMindMapProps> = ({ onBack, initialData, onSave }) => {
  ...
  const [nodeMedia, setNodeMedia] = useState<{type: 'image' | 'video' | 'link', url: string, title?: string}[]>([]);
  ...
  // Handle mouse down for connection dragging
  const handleNodeMouseDown = (e: React.MouseEvent, nodeId: string, nodeX: number, nodeY: number) => {
    if (e.button !== 0) return; // Only left mouse button
    
    // Set up for potentially dragging a connection
    setConnectionStart({id: nodeId, x: nodeX, y: nodeY});
    
    // Add document-wide handlers for mouse move and up
    document.addEventListener('mousemove', handleMouseMove);
    document.addEventListener('mouseup', handleMouseUp);
  };
  
  // Track mouse movement for connection dragging
  const handleMouseMove = (e: MouseEvent) => {
    if (!connectionStart || !svgRef.current) return;
  ...
  // Finish connection dragging or cancel
  const handleMouseUp = (e: MouseEvent) => {
    document.removeEventListener('mousemove', handleMouseMove);
    document.removeEventListener('mouseup', handleMouseUp);
    
    // If we were dragging but not over a node, offer to create a new node
    if (isDraggingConnection && connectionEnd && connectionStart && svgRef.current) {
      const svgRect = svgRef.current.getBoundingClientRect();
      const x = e.clientX - svgRect.left;
      const y = e.clientY - svgRect.top;
  ...
  const handleUpdateNodeDetails = () => {
    ...
    console.log('Updated node with media:', nodeMedia);
