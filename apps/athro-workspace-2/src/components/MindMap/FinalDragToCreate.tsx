...
const nodeTypes: NodeTypes = { simpleNode: CustomNode };
...
function FlowWithDragToCreate() {
  ...
  const onConnectStart = useCallback((event: any, { nodeId, handleId }: { nodeId: string, handleId: string }) => {
    ...
    (event: MouseEvent | TouchEvent) => {
      ...
      if (!targetIsNode) {
        ...
        const newEdge: Edge = {
          ...
          source: connectingNodeRef.current.nodeId as string,
          sourceHandle: connectingNodeRef.current.handleId as string,
          target: newId,
          type: 'smoothstep',
          markerEnd: {
...
// Properly wrapped component with ReactFlowProvider
function FinalDragToCreate() {
  return (
    <ReactFlowProvider>
      <div style={{ width: '100%', height: '100%' }}>
        <FlowWithDragToCreate />
      </div>
    </ReactFlowProvider>
  );
}

export default FinalDragToCreate;
