...
const Dashboard: React.FC<DashboardProps> = ({
  ...
  // Model preference state - Full tier defaults to GPT-4.1, others locked to mini
  const [modelPreference, setModelPreference] = useState<'gpt-4.1' | 'gpt-4o-mini'>('gpt-4o-mini');
  ...
  // Load model preference with tier-based defaults
  const loadModelPreference = async () => {
    ...
    try {
      ...
      if (userTier === 'full') {
        // Full tier: Load preference or default to GPT-4.1 Advanced
        userPreferencesService.setUser(user);
        const preference = await userPreferencesService.getModelPreference();
        const selectedModel = preference || 'gpt-4.1'; // Default to advanced for Full tier
        setModelPreference(selectedModel);
        console.log(`✅ Full tier model preference: ${preference ? preference : 'gpt-4.1 (default)'}`);
      } else {
        // Free/Lite tiers: Locked to mini
        setModelPreference('gpt-4o-mini');
        console.log(`🔒 ${userTier} tier locked to GPT-4o Mini`);
      }
    } catch (error) {
      console.error('Failed to load model preference:', error);
      // Tier-based fallback
      const fallback = userTier === 'full' ? 'gpt-4.1' : 'gpt-4o-mini';
      setModelPreference(fallback);
      console.log(`⚠️ Using fallback for ${userTier} tier: ${fallback}`);
    } finally {
      setModelPreferenceLoading(false);
    }
  };

  // Handle model preference change - Full tier only
  const handleModelPreferenceChange = async (event: React.ChangeEvent<HTMLInputElement>) => {
    // Enforce: Only Full tier users can change model preference
    if (!user || userTier !== 'full') {
      console.warn(`🔒 Model change blocked: ${userTier} tier users cannot change model preference`);
      return;
    }
    ...
    try {
      ...
      setSettingsSuccess(`✅ Model preference updated to ${newModel === 'gpt-4.1' ? 'GPT-4.1 Advanced (Premium)' : 'GPT-4o Mini (Efficient)'}!`);
      console.log(`✅ Full tier model preference updated to: ${newModel}`);
    } catch (error) {
      console.error('Failed to save model preference:', error);
      setSettingsError('Failed to save model preference. Please try again.');
    } finally {
      setModelPreferenceLoading(false);
    }
  };
                  ...
                  {activeSettingsTab === 0 && (
                    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
                      ...
                      <Box sx={{ 
                        ...
                        <Typography variant="subtitle2" sx={{ color: '#e4c97e', fontWeight: 600, mb: 1 }}>
                          🤖 AI Model Selection
                        </Typography>
                        <Typography variant="body2" sx={{ color: '#b5cbb2', mb: 2 }}>
                          {userTier === 'full' 
                            ? 'Choose your preferred AI model for chat interactions:' 
                            : 'Your tier determines the AI model used for chat:'
                          }
                        </Typography>
                        ...
                        <FormControlLabel
                          control={
                            <Switch
                              checked={modelPreference === 'gpt-4.1'}
                              onChange={handleModelPreferenceChange}
                              disabled={userTier !== 'full' || modelPreferenceLoading}
                              color="primary"
                              sx={{
                          ...
                          label={
                            <Box>
                              <Typography variant="body2" sx={{ color: userTier === 'full' ? '#e4c97e' : '#999' }}>
                                {modelPreference === 'gpt-4.1' ? 'GPT-4.1 Advanced (Premium)' : 'GPT-4o Mini (Efficient)'} 
                                {modelPreferenceLoading && ' (Saving...)'}
                              </Typography>
                              {userTier === 'full' && (
                                <Typography variant="caption" sx={{ color: '#b5cbb2', display: 'block' }}>
                                  {modelPreference === 'gpt-4.1' 
                                    ? 'Using premium model - slower but more accurate' 
                                    : 'Using efficient model - faster with lower token usage'
                                  }
                                </Typography>
                              )}
                            </Box>
                        ...
                        {userTier !== 'full' && (
                          <Alert severity="info" sx={{ 
                            ...
                            '& .MuiAlert-message': { fontSize: '0.85rem', color: '#b5cbb2' }
                          }}>
                            <Typography variant="body2" sx={{ fontWeight: 600, mb: 0.5, color: '#2196f3' }}>
                              🔒 Tier Restriction
                            </Typography>
                            <Typography variant="caption" sx={{ color: '#b5cbb2' }}>
                              Free & Lite tiers use GPT-4o Mini for chat efficiency. 
                              Upgrade to Full tier to access GPT-4.1 Advanced with model selection.
                            </Typography>
                          </Alert>
                        )}
